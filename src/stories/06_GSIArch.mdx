...
解像度（1ピクセルあたりの実距離）は、赤道において最大となり、緯度が高くなるにつれて小さくなります。Web Mercator における解像度 $R_{ground}$ (meter/pixel) は以下の式で求められます（タイルサイズ 256px の場合）。

```latex
R_{ground} = \frac{\text{Earth Circumference} \times \cos(\text{latitude})}{2^{\text{zoom}} \times 256}
```
...
- **Z=0**: $156,543 \text{ m/px}$ (国全体)
- **Z=10**: $152.8 \text{ m/px}$ (都市全体)
- **Z=18**: $0.597 \text{ m/px}$ (車一台分)
...
- **緯度0度**: 拡大率 1.0
- **緯度60度**: 拡大率 2.0 (面積は4倍)
- **緯度85度**: 拡大率 $\approx$ 11.5 (面積は100倍以上)

北海道（北緯43度）と沖縄（北緯26度）では、同じズームレベルでも縮尺（画面上の1cmが表す実距離）が異なることに注意が必要です。

## 4. GSI タイルサービス仕様一覧

| サービス | URLテンプレート | データ形式 | 用途と特徴 |
|---------|-----|--------|------|
| **標準地図** | `std/{z}/{x}/{y}.png` | Raster (PNG) | 道路、建物、等高線を含む基本図。注記が焼付けられているため動的なスタイル変更不可。 |
| **淡色地図** | `pale/{z}/{x}/{y}.png` | Raster (PNG) | データの視認性を高めるため、彩度を落とした背景地図。 |
| **航空写真** | `seamlessphoto/...` | Raster (JPEG) | 複数の時期・ソースの写真を色調補正して接合したモザイク画像。 |
| **Kokuarea** | `kokuarea/{z}/{x}/{y}.geojson` | Vector (GeoJSON) | 航空法制限表面。クライアント側でのクリック判定やスタイリングが可能。 |
