...
気象情報は、飛行の可否を決定する最もクリティカルな要素です。本ライブラリは2つのAPIを統合しています。
...
| `C` | 雨雲レーダーのトグル |
| `W` | 天気予報クリックモードのトグル |
| `ESC` | ポップアップ・パネルを順番に閉じる |

---

### RainViewerService (雨雲レーダー)

世界中の気象レーダーを統合したAPI（[RainViewer API](https://www.rainviewer.com/api.html)）。

**ファイル**: `src/lib/services/rainViewer.ts`
...
### WeatherApiService (天気予報)

[Open-Meteo API](https://open-meteo.com/) を使用した都道府県別天気予報サービス。完全無料・認証不要で動作します。

**ファイル**: `src/lib/services/weatherApi.ts`
