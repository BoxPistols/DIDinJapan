...
完全無料のAPIを活用したドローン飛行計画支援システム：

- **天気予報オーバーレイ**
  - 気象庁メッシュ情報API連携（5分間隔、最大72時間先）
  - 風速・風向・降水確率の表示
  - 時間スライダー（5分ステップ）
  
- **飛行安全性チェック**
  - 風速チェック（10 m/s以上で飛行不可）
  - 降水確率チェック（50%以上で警告）
  - LTE通信カバレッジチェック
  - 日没・薄明時間チェック（民間薄明終了後は飛行不可）
...
- OpenSignal/CellMapper（視覚参照）

詳細は [docs/api/DRONE_OPERATION_API_GUIDE.md](./docs/api/DRONE_OPERATION_API_GUIDE.md) を参照。
