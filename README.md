...
Waypoint・飛行経路・飛行範囲が禁止エリア（DID、空港周辺空域、レッドゾーン等）と重複していないかをリアルタイムで検出します。

| 検出対象 | 説明 |
|----------|------|
| **Waypoint** | 単一ポイントが禁止エリア内にあるか判定 |
| **飛行経路** | LineStringが禁止エリアと交差するか判定 |
| **飛行範囲** | Polygonが禁止エリアと重複する面積・割合を計算 |

**ゾーンタイプと深刻度:**

| ゾーン | 色 | 深刻度 |
|--------|-----|--------|
| DID（人口集中地区） | 赤 | WARNING（許可取得で飛行可能） |
| 空港周辺空域 | 紫 | DANGER（原則飛行禁止） |
| レッドゾーン | 暗い赤 | DANGER（原則飛行禁止） |
| イエローゾーン | 黄色 | WARNING（注意が必要） |

**パフォーマンス最適化:**
- RBush空間インデックスによる高速検索（O(log n)）
- 大量のDIDポリゴン（数万件）でも即座に判定可能
