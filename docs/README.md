...
### [飛行注意区域（DID）データ更新ガイド](./data/DID_DATA_UPDATE_GUIDE.md)

e-Stat（政府統計の総合窓口）から人口集中地区（DID）データを取得し、MapLibreで使用可能なGeoJSON形式に変換するまでの手順を記載します。パフォーマンス向上のため、地方ごとに分類されており、必要な地域だけを選択して表示することを推奨します。
